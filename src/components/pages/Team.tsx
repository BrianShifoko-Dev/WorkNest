...
export function Team({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFF0]">
      <Breadcrumbs items={[{ name: "Discover Us" }, { name: "The Team" }]} onNavigate={onNavigate} />

      {/* Hero */}
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 text-center">
          <div className="w-16 h-1 bg-[#D4AF37] mb-6 mx-auto" />
          <h1 className="text-5xl text-[#5C4033] mb-6">Meet Our Team</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {teamMembers.map((member, index) => (
              ...
              >
                ...
                <div className="p-6">
                  <h3 className="text-[#5C4033] mb-1">{member.name}</h3>
                  <p className="text-sm text-[#D4AF37] mb-3">{member.role}</p>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center bg-[#FFFFF0] p-12 rounded-lg border border-[#5C4033]/10">
            <h2 className="text-[#5C4033] mb-4">Join Our Team</h2>
            <p className="text-[#5C4033]/70 mb-6">
              We're always looking for talented individuals who share our passion for excellence 
              and community building.
            </p>
            <a
              href="mailto:careers@finaleworkspace.co.ke"
              className="inline-block px-8 py-3 bg-[#D4AF37] hover:bg-[#B8941F] text-[#5C4033] rounded-lg transition-colors"
            >
              View Open Positions
            </a>
          </div>
